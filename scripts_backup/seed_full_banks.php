...
$banks = [
    ...
    ['name' => 'Sluggish Bank', 'code' => '90001', 'slug' => 'sluggish-bank-not-real-just-padding'], // Checking for obscure ones
...
echo "Bypassing network manual seed (Expanded List)..." . PHP_EOL;

foreach ($banks as $bank) {
    // Upsert logic: Update if exists, insert if not
    $exists = DB::table('unified_banks')->where('code', $bank['code'])->exists();

    if ($exists) {
        DB::table('unified_banks')->where('code', $bank['code'])->update([
            'name' => $bank['name'],
            'paystack_code' => $bank['code'],
            'xixapay_code' => $bank['code'],
            'updated_at' => Carbon::now(),
        ]);
        // echo "Updated: " . $bank['name'] . PHP_EOL;
    } else {
