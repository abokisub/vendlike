...
{
    /**
     * Resolve user from Authorization header with Sanctum support
     */
    private function resolveUserFromHeader($request)
    {
        $authHeader = $request->header('Authorization');
        if (!$authHeader)
            return null;

        // Strip Bearer/Token prefix if present
        ...
        if (!$user && strpos($authHeader, '|') !== false) {
            $userId = $this->verifyapptoken($authHeader);
            if ($userId) {
        ...
        if ($user) {
            ...
            if ($user_info && ($user_info->status == 1 || $user_info->username == 'Habukhan' || $user_info->username == 'developer')) {
                ...
                $user_details = [
                    ...
                    'palmpay' => $xixapay_enabled ? $user->palmpay : null,
                    'pointwave' => $pointwave_enabled ? $user->pointwave_account_number : null,
                    'pointwave_bank' => $pointwave_enabled ? ($user->pointwave_bank_name ?? 'PalmPay') : null,
                    'nin' => $user->nin,
                    'bvn' => $user->bvn,
                    'dob' => $user->dob,
                    'next_of_kin' => json_decode($user->next_of_kin, true),
    ...
    public function Transaction(Request $request)
    {
        $user = $this->resolveUserFromHeader($request);
        if ($user) {
            // Use the resolved user directly instead of re-verifying via request ID
            if ($user->status == 1) {
                // $user is already the correct user object from resolveUserFromHeader
                $trans_history = [];
                $data_trans = [];
                $airtime_trans = [];
                $cable_trans = [];
