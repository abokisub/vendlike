...
echo "ID card exists: " . (file_exists($idCardPath) ? 'YES' : 'NO') . "\n";
...
$guzzle = new \GuzzleHttp\Client(['timeout' => 60, 'http_errors' => false]);

// Try update endpoint — returns customer_id for existing customers
$resp = $guzzle->post('https://api.xixapay.com/api/customer/update', [
    'headers' => ['Authorization' => 'Bearer ' . $secretKey, 'api-key' => $apiKey],
    'multipart' => $multipart,
]);
$data = json_decode($resp->getBody()->getContents(), true);
echo "Update response: " . json_encode($data) . "\n";

$customerId = $data['customer']['customer_id'] ?? $data['data']['customer_id'] ?? null;
...
if ($customerId) {
    ...
    echo "SUCCESS! Saved customer_id: $customerId\n";
} else {
    echo "Could not extract customer_id from response above.\n";
}
